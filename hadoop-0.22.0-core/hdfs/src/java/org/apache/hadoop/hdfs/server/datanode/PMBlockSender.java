 ...
 */
class PMBlockSender implements java.io.Closeable, FSConstants {
    
  native void pmREncode(byte[] data, byte[] code, int bufsize, int mPlusk, int k, int n, int[] fails, ByteBuffer reserve);

  static{
    System.loadLibrary("pmrencode");
  }
  ...
  PMBlockSender(Block block, long startOffset, long length,
  ...
  PMBlockSender(Block block, long startOffset, long length,
    ...
    /** packet size for pm encoding */
    pmPacketSize = datanode.getConf().getInt("hdfs.raid.packet.size", 1024*1024);
    //LOG.info("pmPacketSize: "+pmPacketSize);
    cache = null;
    ...
    try {
      ...
      //assume pm packet is a multiple of bytesPerChecksum
      //offset = (startOffset - (startOffset % bytesPerChecksum));
      offset = (startOffset - (startOffset % (pmPacketSize * n/m)));
      realOffset = realStartOffset - (realStartOffset % pmPacketSize);

      if (length >= 0) {
        // Make sure endOffset points to end of a <del>checksumed chunk</del> pm packet.
        ...
        /*
           ...
           */
        if (tmpLen % (pmPacketSize*n/m) != 0) {
          tmpLen += (pmPacketSize*n/m - tmpLen % (pmPacketSize*n/m));
        }
        if (tmpLen < endOffset) {
  ...
  long sendBlock(DataOutputStream out, OutputStream baseStream, 
    ...
    try {
      ...
      //Encode thread
      PMREncoder encoder = new PMREncoder(q);
  ...
  class PMREncoder implements Runnable{

    private final BlockingQueue<ByteBuffer> queue;
    byte[] checksumBuf = null;
    byte[] buf = null;
    byte[] code = null;
    PMREncoder(BlockingQueue<ByteBuffer> q) {
      queue = q;
      if(checksumSize > 0 && checksumIn != null)
        checksumBuf = new byte[pmPacketSize/bytesPerChecksum*checksumSize];
      buf = new byte[pmPacketSize];
      code = new byte[pmPacketSize*n/m];
    }
    public void run() {
      int iteration = 0;
      LOG.info("pmPacketSize: "+pmPacketSize);
      LOG.info("checksumSize: "+checksumSize);
      LOG.info("bytesPerChecksum: "+bytesPerChecksum);
      LOG.info("realEndOffset: "+realEndOffset);
      LOG.info("realOffset: "+realOffset);
      //if(iteration == 0)
      //  return;
      while(realEndOffset > realOffset) {
        ...
        boolean lastDataPacket = realOffset + pmPacketSize == realEndOffset;
        if(checksumIn != null){
          try{
            //read checksum
        ...
        try{
          //read data
          IOUtils.readFully(blockIn, buf, 0, pmPacketSize);
        }catch(IOException e){
        ...
        if (verifyChecksum) {
          int dOff = 0;
          int cOff = 0;
          int dLeft = pmPacketSize;

          //LOG.info("verify checksum");

          for (int i = 0; i < pmPacketSize/bytesPerChecksum; i++){
            checksum.reset();
            int dLen = Math.min(dLeft, bytesPerChecksum);
            //potential bug here
            checksum.update(buf, dOff, dLen);
            if(!checksum.compare(checksumBuf, cOff)){
              long failedPos = realOffset + pmPacketSize - dLeft;
        ...
        //encode data
        LOG.info("buf size:"+buf.length);
        LOG.info("code size:"+code.length);
        LOG.info("pmPacketSize:"+pmPacketSize);
        LOG.info("k:"+k);
        LOG.info("m:"+m);
        LOG.info("n:"+n);
        ...
        pmREncode(buf, code, pmPacketSize, m+k, k, n, fails, reserve);
