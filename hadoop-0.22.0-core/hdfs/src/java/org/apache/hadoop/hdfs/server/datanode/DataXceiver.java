...
class DataXceiver extends DataTransferProtocol.Receiver
  ...
  public DataXceiver(Socket s, DataNode datanode, 
    ...
    bandwidthPerSec = datanode.getConf().getLong(
        "hdfs.bdr.datanode.throttle",
        1024*1024*1024);
    String localThrottleProperty = "hdfs.bdr.datanode.throttle."
      +s.getLocalAddress().getHostAddress();
        bandwidthPerSec = datanode.getConf().getLong(
        localThrottleProperty, bandwidthPerSec);
    LOG.info("local property "+localThrottleProperty+": "+bandwidthPerSec);
  ...
  /**
   * Read a block from the disk and then encode it.
   */
  @Override
  protected void opReadEncodedBlock(DataInputStream in, Block block,
      long startOffset, long length, String clientName,
      Token<BlockTokenIdentifier> blockToken,
      int k, int m, int n, int[] fails) throws IOException {
    LOG.info("anchor Process_OP_ENCODED_READ");

    //build data output stream
    ...
    // send the block
    PMBlockSender blockSender = null;
    final String clientTraceFmt =
      clientName.length() > 0 && ClientTraceLog.isInfoEnabled()
        ? String.format(DN_CLIENTTRACE_FORMAT, localAddress, remoteAddress,
            "%d", "HDFS_Encoded_READ", clientName, "%d",
    ...
    try {
      try {
        LOG.info("about to send block "+ block.getBlockId() + "with startOffset " +
            startOffset + " length " + length + " m " + m + " k " + k + " n " + n);
        LOG.info("anchor Send_block "+block.getBlockId()+" from "+localAddress
          +" to "+remoteAddress);
        //initial block sender
        blockSender = new PMBlockSender(block, startOffset, length,
            true, true, false, datanode, clientTraceFmt, k, m, n, fails);
        blockSender.setReserve(dataXceiverServer.getReserve(k, m, "pm"));
      } catch(IOException e) {
      ...
      //send block
      //insert a throttler
      DataTransferThrottler throttler = null;//new DataTransferThrottler(10*1024*1024);
      long read = blockSender.sendBlock(out, baseStream, throttler); // send data
    ...
    LOG.info("anchor Finish_OP_ENCODED_READ");
  }
   ...
   */
  @Override
  protected void opReadIAEncodedBlock(DataInputStream in, Block block,
      long startOffset, long length, String clientName,
      Token<BlockTokenIdentifier> blockToken,
      int k, int m, int n, int[] fails) throws IOException {
    LOG.info("anchor Process_OP_IA_ENCODED_READ");
    ...
    // send the block
    IABlockSender blockSender = null;
    final String clientTraceFmt =
      clientName.length() > 0 && ClientTraceLog.isInfoEnabled()
        ? String.format(DN_CLIENTTRACE_FORMAT, localAddress, remoteAddress,
            "%d", "HDFS_Encoded_READ", clientName, "%d",
    ...
    try {
      try {
        ...
        LOG.info("anchor Send_block "+block.getBlockId()+" from "+localAddress
          +" to "+remoteAddress);
        //initial block sender
        blockSender = new IABlockSender(block, startOffset, length,
            true, true, false, datanode, clientTraceFmt, k, m, n, fails);
        blockSender.setReserve(dataXceiverServer.getReserve(k, m, "ia"));
      } catch(IOException e) {
        sendResponse(s, ERROR, datanode.socketWriteTimeout);
        throw e;
      }
  ...
  protected void opReadBlock(DataInputStream in, Block block,
    ...
    LOG.info("anchor Process_OP_READ");
    OutputStream baseStream = NetUtils.getOutputStream(s, 
        datanode.socketWriteTimeout);
    DataOutputStream out = new DataOutputStream(
                 new BufferedOutputStream(baseStream, SMALL_BUFFER_SIZE));
    ...
    final String clientTraceFmt =
      clientName.length() > 0 && ClientTraceLog.isInfoEnabled()
        ? String.format(DN_CLIENTTRACE_FORMAT, localAddress, remoteAddress,
            "%d", "HDFS_NORMAL_READ", clientName, "%d",
            datanode.dnRegistration.getStorageID(), block, "%d")
        : datanode.dnRegistration + " Served block " + block + " to " +
            s.getInetAddress();
    ...
    try {
      ...
      long read = blockSender.sendBlock(out, baseStream, null);//throttler); // send data
    ...
    updateCounter(datanode.myMetrics.readsFromLocalClient,
                  datanode.myMetrics.readsFromRemoteClient);
    LOG.info("anchor Finish_OP_READ");
  }
