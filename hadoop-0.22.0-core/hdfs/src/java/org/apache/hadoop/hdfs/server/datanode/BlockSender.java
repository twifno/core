...
class BlockSender implements java.io.Closeable, FSConstants {
  ...
  BlockingQueue<byte[]> q = new ArrayBlockingQueue<byte[]>(1000);
  ...
  BlockSender(Block block, long startOffset, long length,
    ...
    pmPacketSize = datanode.getConf().getInt("dfs.pm.packet.size", 1024*1024);
    ...
    try {
      ...
      readOffset = (startOffset - (startOffset % bytesPerChecksum));
      if (length >= 0) {
  ...
  private int sendChunks(ByteBuffer pkt, int maxChunks, OutputStream out) 
    ...
    //LOG.info("anchor Send_packet "+seqno+" Checksum_read");
    ...
    if (blockInPosition < 0) {
     ...
     try{
        if(tmpBuf == null)
          tmpBuf = q.take();
        int read = 0;
        if(tmpBuf.length - bufOffset >= len){
          System.arraycopy(tmpBuf, bufOffset, buf, dataOff, len);
          bufOffset += len;
        }else{
          read = bufOffset - tmpBuf.length;
          System.arraycopy(tmpBuf, bufOffset, buf, dataOff, read);
          tmpBuf = q.take();
          System.arraycopy(tmpBuf, 0, buf, dataOff+read, len-read);
          bufOffset = len-read;
        }
      }catch(InterruptedException ex) {
        LOG.info("Interrupted Exception when getting new data");
      }
      //LOG.info("anchor Send_packet "+seqno+" Data_read");
    ...
    //LOG.info("anchor Send_packet "+seqno+" Checksum_checked");
    ...
    try {
      ...
      //LOG.info("anchor Send_packet "+seqno+" Data_sent");
  ...
  long sendBlock(DataOutputStream out, OutputStream baseStream, 
    ...
    try {
      ...
      DiskDataReader ddr = new DiskDataReader(maxChunksPerPacket);
      new Thread(ddr).start();
  ...
  class DiskDataReader implements Runnable{
    private int maxChunks;

    DiskDataReader(int m){
      maxChunks = m;
    }

    public void run(){
      while(endOffset > readOffset){
        int len = (int) Math.min(endOffset - readOffset,
            (((long) bytesPerChecksum) * ((long) maxChunks))*100);
        byte[] buf = new byte[len];
        try{
          IOUtils.readFully(blockIn, buf, 0, len);
        }catch(IOException e){
          LOG.warn(" Could not data" +
              " at offset " + readOffset + " for block " + block + " got : "
              + StringUtils.stringifyException(e) +
              "in block sender");
          IOUtils.closeStream(blockIn);
          blockIn = null;
        ...
        try{
          ...
          LOG.info("Interrupted Exception when putting new data");
        }
        readOffset += len;
      }
    }
