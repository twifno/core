...
public class BlockReader extends FSInputChecker {
  ...
  public synchronized int read(byte[] buf, int off, int len) 
    ...
    if (lastChunkLen < 0 && startOffset > firstChunkOffset && len > 0) {
      ...
      int testret;
      if ( (testret = super.read(skipBuf, 0, toSkip)) != toSkip ) {
        // should never happen
        LOG.info("Could not skip required number of bytes: "+toSkip + " with return value: "+testret);
   ...
   * @param k k
   * @param m m
   * @param n num fail blocks
   * @param fails fail index
  ...
  public static BlockReader newBlockReader( Socket sock, String file,
    ...
    DataTransferProtocol.Sender.opReadEncodedBlock(
        new DataOutputStream(new BufferedOutputStream(
            NetUtils.getOutputStream(sock,HdfsConstants.WRITE_TIMEOUT))),
        block, startOffset, len, clientName, blockToken, k, m, n, fails);
  ...
  public static BlockReader newBlockReader( Socket sock, String file,
    ...
    // in and out will be closed when sock is closed (by the caller)
    if(code.equals("ia")){
      DataTransferProtocol.Sender.opReadIAEncodedBlock(
          new DataOutputStream(new BufferedOutputStream(
              NetUtils.getOutputStream(sock,HdfsConstants.WRITE_TIMEOUT))),
          block, startOffset, len, clientName, blockToken, k, m, n, fails);
    }else{
