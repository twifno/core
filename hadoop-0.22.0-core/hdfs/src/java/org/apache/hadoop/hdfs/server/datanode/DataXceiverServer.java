...
class DataXceiverServer implements Runnable, FSConstants {
  ...
  DataXceiverServer(ServerSocket ss, Configuration conf, 
    ...
    LOG.info("DataXceiverServer is contructed");
    getReserve(0, 0, "");
    LOG.info("encoder is initialized");
  }
  ...
  private Map<String, ByteBuffer> reserves = null;

  native void pminit(int n, int k, ByteBuffer reserve);
  native void iainit(int n, int k, ByteBuffer reserve);

  
  static{
    System.loadLibrary("pminit");
    System.loadLibrary("iainit");
  }
  

  public ByteBuffer getReserve(int k, int m, String code){
    String key = "k="+k+"m="+m+code;
    if(reserves == null){
      reserves = new HashMap<String, ByteBuffer>();
      ByteBuffer reserve = ByteBuffer.allocateDirect(1024);
      pminit(5, 3, reserve);
      reserves.put("k=3m=2pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(9, 5, reserve);
      reserves.put("k=5m=4pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(15, 8, reserve);
      reserves.put("k=8m=7pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(17, 9, reserve);
      reserves.put("k=9m=8pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(13, 7, reserve);
      reserves.put("k=7m=6pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(11, 6, reserve);
      reserves.put("k=6m=5pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(7, 4, reserve);
      reserves.put("k=4m=3pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(9, 4, reserve);
      reserves.put("k=4m=5pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(16, 8, reserve);
      reserves.put("k=8m=8pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      pminit(20, 10, reserve);
      reserves.put("k=10m=10pm", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      iainit(9, 4, reserve);
      reserves.put("k=4m=5ia", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      iainit(16, 8, reserve);
      reserves.put("k=8m=8ia", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      iainit(20, 10, reserve);
      reserves.put("k=10m=10ia", reserve);
      reserve = ByteBuffer.allocateDirect(1024);
      iainit(12, 6, reserve);
      reserves.put("k=6m=6ia", reserve);
    }
