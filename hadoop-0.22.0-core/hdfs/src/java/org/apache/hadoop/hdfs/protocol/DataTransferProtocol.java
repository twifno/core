...
/*
 * add one more request type READ_ENCODED_BLOCK
 * added by linjian
 * time: 19/06/2012
 */
/*
 * add one more request type READ_IA_ENCODED_BLOCK
 * added by linjian
 * time: 14/01/2013
 */
...
public interface DataTransferProtocol {
  ...
  public enum Op {
    ...
    READ_ENCODED_BLOCK((byte)86),
    READ_IA_ENCODED_BLOCK((byte)87);
  ...
  /** @deprecated Deprecated at 0.21.  Use Op.READ_ENCODED_BLOCK instead. */
  @Deprecated
  public static final byte OP_READ_ENCODED_BLOCK = Op.READ_ENCODED_BLOCK.code;
  /** @deprecated Deprecated at 0.21.  Use Op.READ_IA_ENCODED_BLOCK instead. */
  @Deprecated
  public static final byte OP_READ_IA_ENCODED_BLOCK = Op.READ_IA_ENCODED_BLOCK.code;
  ...
  public static class Sender {
    ...
    /** Send OP_READ_ENCODED_BLOCK */
    public static void opReadEncodedBlock(DataOutputStream out, Block blk,
        long blockOffset, long blockLen, String clientName,
        Token<BlockTokenIdentifier> blockToken,
        int k, int m, int n, int[] fails)
      throws IOException {
      op(out, Op.READ_ENCODED_BLOCK);
      ...
      out.writeInt(n);
      for(int i = 0; i<n; i++){
        out.writeInt(fails[i]);
      }
      out.flush();
    }

    /** Send OP_READ_IA_ENCODED_BLOCK */
    public static void opReadIAEncodedBlock(DataOutputStream out, Block blk,
        long blockOffset, long blockLen, String clientName,
        Token<BlockTokenIdentifier> blockToken,
        int k, int m, int n, int[] fails)
      throws IOException {
      op(out, Op.READ_IA_ENCODED_BLOCK);
  ...
  public static abstract class Receiver {
    ...
    protected final void processOp(Op op, DataInputStream in
      ...
      case READ_BLOCK:
        opReadBlock(in);
        break;
      case READ_ENCODED_BLOCK:
        opReadEncodedBlock(in);
        break;
      case READ_IA_ENCODED_BLOCK:
        opReadIAEncodedBlock(in);
        break;
      case WRITE_BLOCK:
        opWriteBlock(in);
        break;
    ...
    /** Receive OP_READ_ENCODED_BLOCK */
    private void opReadEncodedBlock(DataInputStream in) throws IOException {
      ...
      opReadEncodedBlock(in, blk, offset, length, client, blockToken, k, m, n, fails);
    }

    /**
     * Abstract OP_READ_ENCODED_BLOCK method. Read a block.
     */
    protected abstract void opReadEncodedBlock(DataInputStream in, Block blk,
        long offset, long length, String client,
        Token<BlockTokenIdentifier> blockToken,
        int k, int m, int n, int[] fails) throws IOException;
    
    /** Receive OP_READ_IA_ENCODED_BLOCK */
    private void opReadIAEncodedBlock(DataInputStream in) throws IOException {
      final Block blk = new Block();
      blk.readId(in);
      final long offset = in.readLong();
      final long length = in.readLong();
      ...
      opReadIAEncodedBlock(in, blk, offset, length, client, blockToken, k, m, n, fails);
    }

    /**
     * Abstract OP_READ_IA_ENCODED_BLOCK method. Read a block.
     */
    protected abstract void opReadIAEncodedBlock(DataInputStream in, Block blk,
        long offset, long length, String client,
        Token<BlockTokenIdentifier> blockToken,
        int k, int m, int n, int[] fails) throws IOException;
