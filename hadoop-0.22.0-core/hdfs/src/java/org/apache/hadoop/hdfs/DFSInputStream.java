...
public class DFSInputStream extends FSInputStream {
  ...
  DFSInputStream(DFSClient dfsClient, String src, int buffersize, boolean verifyChecksum,
      int k, int m, int n, int[]  fails, String code
                 ) throws IOException, UnresolvedLinkException {
  ...
  private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
    ...
    while (true) {
      ...
      try {
        ...
        /*** ***/
        long realOffset = offsetIntoBlock;
        long realLen = blk.getNumBytes();
        if(requireEncode()){
          realLen = realLen * n / m;
        }
        realLen -= offsetIntoBlock;
  ...
  private synchronized int readBuffer(byte buf[], int off, int len) 
    ...
    while (true) {
      ...
      try {
        //DFSClient.LOG.info("DFSInputStream off: "+off+" len: "+len);
      ...
      if (retryCurrentNode) {
         ...
         */ 
        DFSClient.LOG.info("Need to retry current node");
    ...
    if (pos < getFileLength()) {
      ...
      while (retries > 0) {
        try {
          ...
          //DFSClient.LOG.info("result for readBuffer: "+result);
          if (result >= 0) {
            pos += result;
          } else {
  ...
  protected BlockReader getBlockReader(InetSocketAddress dnAddr,
    ...
    for (int retries = 0; retries <= nCachedConnRetry && fromCache; ++retries) {
      ...
      try {
        // The OP_READ_ENCODED_BLOCK/OP_READ_BLOCK request is sent as we make the BlockReader
        BlockReader reader = null;
        if(requireEncode()){
          if(this.code != null)
            reader = BlockReader.newBlockReader(sock, file, block,
                                       blockToken,
                                       startOffset, len,
