...
public class DistributedFileSystem extends FileSystem {
  ...
  public FSDataInputStream open(Path f, int bufferSize, int k, int m, int n, 
      int[] fails, String code)
  throws IOException {
    statistics.incrementReadOps(1);
    return new DFSClient.DFSDataInputStream(
          dfs.open(getPathName(f), bufferSize, verifyChecksum, statistics, k, m, n, 
            fails, code));
  }
  ...
  throws IOException {
    statistics.incrementReadOps(1);
    return new DFSClient.DFSDataInputStream(
          dfs.open(getPathName(f), bufferSize, verifyChecksum, statistics, k, m, n, fails));
  }
