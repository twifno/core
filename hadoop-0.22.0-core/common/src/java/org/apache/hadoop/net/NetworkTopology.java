 ...
 */
/* this file is modified by {lin jian}[jlin@cse.cuhk.edu.hk] */
...
public class NetworkTopology {
  ...
  private class InnerNode extends NodeBase {
    ...
    ArrayList<Node> getLeaves(){
      ...
      for(Iterator<Node> iter = children.iterator();
          iter.hasNext();){
        Node node = iter.next();
        if(!(node instanceof InnerNode))
          ret.add(node);
        else
          ret.addAll(((InnerNode)node).getLeaves());
      }
      return ret;
    }
  ...
  //Derandomized
  final private static Random r = new Random(37);
  ...
  private Node chooseRandom(String scope, String excludedScope){
    ...
    LOG.info("leaveIndex: "+leaveIndex+" with numOfDatanodes: "+numOfDatanodes);
    return innerNode.getLeaf(leaveIndex, node);
  }

  /**
   * choose a node by index
   * return the node with that "index"
   */
  public Node chooseByIndex(int index){
    Node node = getNode(NodeBase.ROOT);
    if (!(node instanceof InnerNode)) {
      return node;
    }   
    InnerNode innerNode = (InnerNode)node;
    int numOfDatanodes = innerNode.getNumOfLeaves();
    node = null;
    int leaveIndex = index % numOfDatanodes;
    LOG.info("leaveIndex: "+leaveIndex+" with numOfDatanodes: "+numOfDatanodes);
    return innerNode.getLeaf(leaveIndex, node);
  }
  ...
  public void pseudoSortByDistance( Node reader, Node[] nodes ) {
    ...
    if(tempIndex == 0 && nodes.length != 0) {
      ...
      LOG.info("nodeIndex: "+nodeIndex+" with nodes.length: "+nodes.length);
      swap(nodes, 0, nodeIndex);
    }
  }
}
