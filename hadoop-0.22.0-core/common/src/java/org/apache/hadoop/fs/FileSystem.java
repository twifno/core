...
public abstract class FileSystem extends Configured implements Closeable {
  ...
  public static FileSystem get(URI uri, Configuration conf) throws IOException {
    ...
    FileSystem tmpfs = CACHE.get(uri, conf);
    //LOG.info("cache fs type: "+tmpfs.getClass().getName());
    return tmpfs;
  }
  ...
  public FSDataInputStream open(Path f, int bufferSize, int k, int m, int n, int[] fails, String code)
    throws IOException{
    //baseline implementation, just ignore those other parameters
    return open(f, bufferSize);
  }

  public FSDataInputStream open(Path f, int bufferSize, int k, int m, int n, int[] fails)
    throws IOException{
    //baseline implementation, just ignore those other parameters
    return open(f, bufferSize);
  }
 ...
 /**
  ...
  private static FileSystem createFileSystem(URI uri, Configuration conf
    ...
    String test = "fs." + uri.getScheme() + ".impl";
    Class<?> clazz = conf.getClass("fs." + uri.getScheme() + ".impl", null);

    //LOG.info("create file system: "+clazz.getName());
    //LOG.info("create file system: "+conf.getClass("fs." + uri.getScheme() + ".impl", null).getName());
    if (clazz == null) {
      throw new IOException("No FileSystem for scheme: " + uri.getScheme());
    }
    FileSystem fs = (FileSystem)ReflectionUtils.newInstance(conf.getClass("fs." + uri.getScheme() + ".impl", null), conf);
    fs.initialize(uri, conf);
    //LOG.info("fs type after create: "+fs.getClass().getName());
    return fs;
  }
  ...
  static class Cache {
    ...
    private FileSystem getInternal(URI uri, Configuration conf, Key key) throws IOException{
      ...
      //LOG.info("created fs: "+fs.getClass().getName());
      synchronized (this) { // refetch the lock again
        FileSystem oldfs = map.get(key);
        if (oldfs != null) { // a file system is created while lock is releasing
          fs.close(); // close the new file system
          //LOG.info("return old fs");
