...
import org.apache.hadoop.raid.IADecoder;
...
public class DistributedRaidFileSystem extends FilterFileSystem {
  ...
  static private class DecodeInfo {
    ...
    Decoder createDecoder() {
      if (this.type == ErasureCodeType.PM) {
        return new PMDecoder(conf, stripeLength,
            RaidNode.pmParityLength(conf), false);
      } else if (this.type == ErasureCodeType.IA) {
        return new IADecoder(conf, stripeLength,
            RaidNode.iaParityLength(conf), false);
      } else if (this.type == ErasureCodeType.JRS) {
        return new JRSDecoder(conf, stripeLength,
            RaidNode.jrsParityLength(conf), false);
      }
      return null;
    }
  }
  ...
  public void initialize(URI name, Configuration conf) throws IOException {
    ...
    alternates= new DecodeInfo[3];
    Path iaPath = RaidNode.iaDestinationPath(conf, fs);
    alternates[0] = new DecodeInfo(conf, ErasureCodeType.IA, iaPath);
    Path jrsPath = RaidNode.jrsDestinationPath(conf, fs);
    alternates[1] = new DecodeInfo(conf, ErasureCodeType.JRS, jrsPath);
    Path pmPath = RaidNode.pmDestinationPath(conf, fs);
    alternates[2] = new DecodeInfo(conf, ErasureCodeType.PM, pmPath);
  }
    ...
    private static class ExtFsInputStream extends FSInputStream {
      ...
      public synchronized int read(byte[] b, int offset, int len) 
        ...
        while (true) {
          ...
          //LOG.info("read openCurrentStream");
          openCurrentStream();
          try{
            int limit = Math.min(blockAvailable(), len);
          //LOG.info("limit: "+limit+" before currentOffset.read");
            int value = currentStream.read(b, offset, limit);
            currentOffset += value;
            nextLocation = 0;
            return value;
          } catch (BlockMissingException e) {
          //  LOG.info("block missing exception 1");
            long start = System.nanoTime();
            setAlternateLocations(e, currentOffset);
            long end = System.nanoTime();
            LOG.info("anchor Degraded_read_latency "+(end-start));
        ...
        try {
          while (true) {
            ...
            try {
              ...
              LOG.info("block missing exception 2");
              setAlternateLocations(e, currentOffset);
            } catch (ChecksumException e) {
              setAlternateLocations(e, currentOffset);
            }
          ...
          try {
            ...
            try{
              ...
              if (outdir != null) {
                ...
                ps.println("Stripe lenght : " + stripeLength);
                ps.println("Corrupt offset : " + corruptOffset+blockSize);
