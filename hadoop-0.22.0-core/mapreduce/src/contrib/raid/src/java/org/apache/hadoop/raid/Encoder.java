...
public abstract class Encoder {
  ...
  public void encodeFile(
    ...
    //make sure that blockSize can be divided by bufSize
    //configureBuffers(blockSize);

    // Create a tmp file to which we will write first.
    // more detail, open an output stream to write parity data.
    // This file is a temp file, when the creation of parity finished, this file
    // will be moved to a permanent place.
    ...
    try {
      LOG.info("start encodeFileToStream");
      encodeFileToStream(fs, srcFile, srcSize, blockSize, out, reporter);
      LOG.info("end encodeFileToStream");
      
      //finished parity creation
      out.close();
      out = null;
      ...
      LOG.info("final encodeFileToStream");
      if (out != null) {
        out.close();
      }
      //delete temp parity file
   ...
   * @param out The destination for the reovered block.
   */
  protected void encodeFileToStream(FileSystem fs, Path srcFile, long srcSize,
    ...
    try {
      ...
      for (long stripeStart = 0; stripeStart < srcSize;
        ...
        for (int i = 0; i < paritySize - 1; i++) {
          LOG.info("anchor Output_parity "+tmpFiles[i].getName()+" Start_output "+System.nanoTime());
          tmpOuts[i + 1].close();
          tmpOuts[i + 1] = null;
          InputStream in  = new FileInputStream(tmpFiles[i]);
          RaidUtils.copyBytes(in, out, writeBufs[i], blockSize);
          reporter.progress();
          LOG.info("anchor Output_parity "+tmpFiles[i].getName()+" End_output "+System.nanoTime());
        }
      }
    } finally {
      for (int i = 0; i < paritySize - 1; i++) {
