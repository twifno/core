...
public class RaidUtils {
  ...
  public static void readTillEnd(InputStream in, byte[] buf, int len, boolean eofOK)
    throws IOException {
    int toRead = len;
    int numRead = 0;
  ...
  public static void readTillEnd(InputStream in, byte[] buf, boolean eofOK, int offset, int length)
    throws IOException {
    int toRead = length;
    int numRead = 0;
    while (numRead < toRead) {
      int nread = in.read(buf, numRead+offset, toRead - numRead);
      if (nread < 0) {
        if (eofOK) {
          // EOF hit, fill with zeros
          Arrays.fill(buf, numRead+offset, toRead+offset, (byte)0);
          numRead = toRead;
        } else {
          // EOF hit, throw.
          throw new IOException("Premature EOF");
