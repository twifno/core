...
public abstract class RaidNode implements RaidProtocol {
  ...
  public static final int DEFAULT_PORT = 30016;
  // Default stripe length = 3, parity length for RS code = 3
  public static final int DEFAULT_STRIPE_LENGTH = 5;

	/** code type specific */
  public static final int PM_PARITY_LENGTH_DEFAULT = 4;
  public static final int IA_PARITY_LENGTH_DEFAULT = 4;
  public static final int JRS_PARITY_LENGTH_DEFAULT = 4;

  public static final String PM_PARITY_LENGTH_KEY = "hdfs.raidpm.paritylength";
  public static final String JRS_PARITY_LENGTH_KEY = "hdfs.raidjrs.paritylength";
  public static final String IA_PARITY_LENGTH_KEY = "hdfs.raidia.paritylength";
  ...
  public static final String DEFAULT_RAIDIA_LOCATION = "/raidia";
  public static final String RAIDIA_LOCATION_KEY = "hdfs.raidia.locations";
  public static final String DEFAULT_RAIDIA_TMP_LOCATION = "/tmp/raidia";
  public static final String RAIDIA_TMP_LOCATION_KEY = "fs.raidia.tmpdir";
  public static final String DEFAULT_RAIDIA_HAR_TMP_LOCATION = "/tmp/raidia_har";
  public static final String RAIDIA_HAR_TMP_LOCATION_KEY = "fs.raidia.hartmpdir";

  public static final String DEFAULT_RAIDJRS_LOCATION = "/raidjrs";
  public static final String RAIDJRS_LOCATION_KEY = "hdfs.raidjrs.locations";
  public static final String DEFAULT_RAIDJRS_TMP_LOCATION = "/tmp/raidjrs";
  public static final String RAIDJRS_TMP_LOCATION_KEY = "fs.raidjrs.tmpdir";
  public static final String DEFAULT_RAIDJRS_HAR_TMP_LOCATION = "/tmp/raidjrs_har";
  public static final String RAIDJRS_HAR_TMP_LOCATION_KEY = "fs.raidjrs.hartmpdir";

  public static final String DEFAULT_RAIDPM_LOCATION = "/raidpm";
  public static final String RAIDPM_LOCATION_KEY = "hdfs.raidpm.locations";
  public static final String DEFAULT_RAIDPM_TMP_LOCATION = "/tmp/raidpm";
  public static final String RAIDPM_TMP_LOCATION_KEY = "fs.raidpm.tmpdir";
  public static final String DEFAULT_RAIDPM_HAR_TMP_LOCATION = "/tmp/raidpm_har";
  public static final String RAIDPM_HAR_TMP_LOCATION_KEY = "fs.raidpm.hartmpdir";
    ...
    DirectoryTraversal.FileFilter filterForPolicy(
      ...
      case JRSO:
        return new RaidFilter.PreferenceFilter(
          conf, pmDestinationPath(conf), jrsDestinationPath(conf),
          info, allPolicies, startTime, stats);
      case PM:
      case PMO:
        return new RaidFilter.TimeBasedFilter(conf, pmDestinationPath(conf),
          info, allPolicies, startTime, stats);
      case IA:
      case IAO:
        return new RaidFilter.TimeBasedFilter(conf, iaDestinationPath(conf),
          info, allPolicies, startTime, stats);
      default:
        return null;
      }
  ...
  static ParityFilePair jrsParityForSource(Path srcPath, Configuration conf)
      throws IOException {
    try {
      Path destPath = jrsDestinationPath(conf);
  ...
  static ParityFilePair pmParityForSource(Path srcPath, Configuration conf)
      throws IOException {
    try {
      Path destPath = pmDestinationPath(conf);
  ...
  static ParityFilePair iaParityForSource(Path srcPath, Configuration conf)
      throws IOException {
    try {
      Path destPath = iaDestinationPath(conf);
      return getParityFile(destPath, srcPath, conf);
    } catch (FileNotFoundException e) {
    }
    return null;
   ...
   * lj: starting point of the migration
   */
   ...
   * RAID an individual file
   * lj: starting point of the migration
   * temporarily set to public for testing
   */
  ...
  /**
   * Return the temp path for jrs parity files
   */
  public static String jrsTempPrefix(Configuration conf) {
    return conf.get(RAIDJRS_TMP_LOCATION_KEY, DEFAULT_RAIDJRS_TMP_LOCATION);
  }

  /**
   * Return the temp path for pm parity files
   */
  public static String pmTempPrefix(Configuration conf) {
    return conf.get(RAIDPM_TMP_LOCATION_KEY, DEFAULT_RAIDPM_TMP_LOCATION);
  }

  /**
   * Return the temp path for ia parity files
   */
  public static String iaTempPrefix(Configuration conf) {
    return conf.get(RAIDIA_TMP_LOCATION_KEY, DEFAULT_RAIDIA_TMP_LOCATION);
  }

  /**
   * Return the temp path for jrs parity files
   */
  public static String jrsHarTempPrefix(Configuration conf) {
    return conf.get(RAIDJRS_HAR_TMP_LOCATION_KEY, DEFAULT_RAIDJRS_HAR_TMP_LOCATION);
  }

  /**
   * Return the temp path for pm parity files
   */
  public static String pmHarTempPrefix(Configuration conf) {
    return conf.get(RAIDPM_HAR_TMP_LOCATION_KEY, DEFAULT_RAIDPM_HAR_TMP_LOCATION);
  }

  /**
   * Return the temp path for ia parity files
   */
  public static String iaHarTempPrefix(Configuration conf) {
    return conf.get(RAIDIA_HAR_TMP_LOCATION_KEY, DEFAULT_RAIDIA_HAR_TMP_LOCATION);
  }

  /**
   * Return the destination path for jrs parity files
   */
  public static Path jrsDestinationPath(Configuration conf, FileSystem fs) {
    String loc = conf.get(RAIDJRS_LOCATION_KEY, DEFAULT_RAIDJRS_LOCATION);
  ...
  /**
   * Return the destination path for pm parity files
   */
  public static Path pmDestinationPath(Configuration conf, FileSystem fs) {
    String loc = conf.get(RAIDPM_LOCATION_KEY, DEFAULT_RAIDPM_LOCATION);
    Path p = new Path(loc.trim());
    p = p.makeQualified(fs);
    return p;
  }
  ...
  /**
   * Return the destination path for ia parity files
   */
  public static Path iaDestinationPath(Configuration conf, FileSystem fs) {
    String loc = conf.get(RAIDIA_LOCATION_KEY, DEFAULT_RAIDIA_LOCATION);
    Path p = new Path(loc.trim());
    p = p.makeQualified(fs);
    return p;
  }
   ...
   * Return the destination path for jrs parity files
   */
  public static Path jrsDestinationPath(Configuration conf)
    throws IOException {
    String loc = conf.get(RAIDJRS_LOCATION_KEY, DEFAULT_RAIDJRS_LOCATION);
    Path p = new Path(loc.trim());
   ...
   * Return the destination path for pm parity files
   */
  public static Path pmDestinationPath(Configuration conf)
    throws IOException {
    String loc = conf.get(RAIDPM_LOCATION_KEY, DEFAULT_RAIDPM_LOCATION);
    Path p = new Path(loc.trim());
    FileSystem fs = FileSystem.get(p.toUri(), conf);
   ...
   * Return the destination path for ia parity files
   */
  public static Path iaDestinationPath(Configuration conf)
    throws IOException {
    String loc = conf.get(RAIDIA_LOCATION_KEY, DEFAULT_RAIDIA_LOCATION);
    Path p = new Path(loc.trim());
    FileSystem fs = FileSystem.get(p.toUri(), conf);
  ...
  static Path getDestinationPath(ErasureCodeType code, Configuration conf)
    ...
    switch (code) {
      case PMO:
      case PM:
        return pmDestinationPath(conf);
      case JRSO:
      case JRS:
        return jrsDestinationPath(conf);
      case IAO:
      case IA:
        return iaDestinationPath(conf);
      default:
        return null;
    }
  }
  ...
  static Encoder encoderForCode(Configuration conf, ErasureCodeType code) {
    ...
    switch (code) {
      case JRSO:
        return new JRSOEncoder(conf, stripeLength, jrsParityLength(conf));
      case JRS:
        return new JRSEncoder(conf, stripeLength, jrsParityLength(conf));
      case PMO: 
      case PM:
        return new PMEncoder(conf, stripeLength, pmParityLength(conf));
      case IAO:
        return new IAOEncoder(conf, stripeLength, iaParityLength(conf));
      case IA:
        return new IAEncoder(conf, stripeLength, iaParityLength(conf));
      default:
        return null;
    }
  }
  ...
  static String tmpHarPathForCode(Configuration conf, ErasureCodeType code) {
    switch (code) {
      case JRSO:
      case JRS:
        return jrsHarTempPrefix(conf);
      case PMO:
      case PM:
        return pmHarTempPrefix(conf);
      case IAO:
      case IA:
        return iaHarTempPrefix(conf);
      default:
        return null;
    }
  }
   ...
   * Obtain stripe length from configuration
   */
  public static int jrsParityLength(Configuration conf) {
    return conf.getInt(JRS_PARITY_LENGTH_KEY, JRS_PARITY_LENGTH_DEFAULT);
  }

  /**
   * Obtain stripe length from configuration
   */
  public static int pmParityLength(Configuration conf) {
    return conf.getInt(PM_PARITY_LENGTH_KEY, PM_PARITY_LENGTH_DEFAULT);
  }

  /**
   * Obtain stripe length from configuration
   */
  public static int iaParityLength(Configuration conf) {
    return conf.getInt(IA_PARITY_LENGTH_KEY, IA_PARITY_LENGTH_DEFAULT);
  }
  ...
  public static FSDataInputStream unRaidParityStream(Configuration conf, 
      Path srcPath, Path destPathPrefix, int stripeIdx, 
      int idx, int paritylength) throws IOException{
    ParityFilePair ppair = getParityFile(destPathPrefix, srcPath, conf);
    //LOG.info("dest prefix "+destPathPrefix);
    //LOG.info("source path "+srcPath);
    //LOG.info("parity stream path "+ppair.getPath());
    if (ppair == null)
      return null;
    long blockSize = ppair.getFileSystem().getFileStatus(ppair.getPath()).getBlockSize();
    FSDataInputStream s = ((DistributedRaidFileSystem)ppair.getFileSystem())
      .getFileSystem().open(ppair.getPath(), 1024);
    s.seek(blockSize*(paritylength*stripeIdx+idx));
    return s;
  }
  ...
  public static Path unRaidCorruptBlock(Configuration conf, Path srcPath,
    ...
    ParityFilePair ppair = getParityFile(destPathPrefix, srcPath, conf);
    if (ppair == null) {
      LOG.warn("Could not find parity file for " + srcPath);
      return null;
    }
    ...
    try {
      LOG.info("Call decoder.fixErasedBlock");
