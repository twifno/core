...
public class PMOEncoder extends Encoder {

	native void pmoEncode(ByteBuffer message, ByteBuffer parity, int k, int m, int bufferSize, ByteBuffer r);

  int seq;

	static{
		System.loadLibrary("pmoencoder");
	}

  public static final Log LOG = LogFactory.getLog(
                                  "org.apache.hadoop.raid.PMOEncoder");
  ...
  void performEncode(byte[][] readBufs, byte[][] writeBufs) {
    ...
    */
    ...
    LOG.info("before doing pmo encode with bufSize" + bufSize + " and array size " + readBufs[0].length);
    LOG.info("anchor Encode_stripe "+seq+" Start_encoding "+System.nanoTime());
    pmoEncode(dataBufs, parityBufs, stripeSize, paritySize, bufSize, reserve);
    LOG.info("anchor Encode_stripe "+seq+" Encoded "+System.nanoTime());
    LOG.info("after doing pmo encode");
    for(int i = 0; i< paritySize; i++){
