...
public class PMEncoder extends Encoder {

  private int threadNum = 2;

  static{
    System.loadLibrary("pmencoder");
  }
  ...
  public static final Log LOG = LogFactory.getLog(
      "org.apache.hadoop.raid.PMEncoder");
  public PMEncoder(
    ...
    //encode thread
    PMMigrationEncoder[] encoder = new PMMigrationEncoder[threadNum];
    Thread[] es = new Thread[threadNum];
    for(int i = 0; i < threadNum; i++){
      encoder[i] = new PMMigrationEncoder(i);
      es[i] = new Thread(encoder[i]);
      es[i].start();
    }

    LOG.info("PMEncoder 10/9/12");
    ...
    public void run(){
      ...
      while(count < mc){
        ...
        try{
          ...
          LOG.info("Unexpected IOException in PMEncode line 140 "+e);
        }
  ...
  @Override
    public Path getParityTempPath() {
      return new Path(RaidNode.pmTempPrefix(conf));
    }

  /*
   * Class to take charged of encoding
   */
  class PMMigrationEncoder implements Runnable{
    ...
    native void pmEncode(ByteBuffer in, ByteBuffer out, int k, int m, int bufSize, ByteBuffer r);

    PMMigrationEncoder(int i){
    ...
    //private?
    public void take(){
      //LOG.info("PMMigrationEncoder take called");
      inBuf.clear();
    ...
    public void push(){
      //LOG.info("PMMigrationEncoder push called strange 3");
    ...
    public void run(){
      LOG.info("run pmmigrationencoder");
      while(true){
        //File[] fs = null;
        try{
          //take signal
        ...
        //}
        pmEncode(inBuf, outBuf, stripeSize, paritySize, bufSize, reserve);
