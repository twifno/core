...
public class IAEncoder extends Encoder {

  private int threadNum = 2;

  static{
    System.loadLibrary("iaencoder");
  }
  ...
  public static final Log LOG = LogFactory.getLog(
      "org.apache.hadoop.raid.IAEncoder");
  public IAEncoder(
    ...
    for(int i=0; i<threadNum; i++)
      q[i] = new ArrayBlockingQueue<ByteBuffer>(1024/paritySize);
    ...
    //encode thread
    IAMigrationEncoder[] encoder = new IAMigrationEncoder[threadNum];
    Thread[] es = new Thread[threadNum];
    for(int i = 0; i < threadNum; i++){
      encoder[i] = new IAMigrationEncoder(i);
      es[i] = new Thread(encoder[i]);
      es[i].start();
    }

    LOG.info("IAEncoder 10/9/12 StripeSize: "+stripeSize+" ParitySize: "+paritySize);
    ...
    public void run(){
      ...
      while(count < mc){
        ...
        try{
          ...
          //LOG.info("output length: "+f.length+" count: "+count);
          out.write(f, 0, f.length);
          //in.close();
          //f.delete();
          //LOG.info("Deleted tmp file " + f);
        }catch(IOException e){
          LOG.info("Unexpected IOException in IAEncode line 140 "+e);
        }
   ...
   */
  class IAMigrationEncoder implements Runnable{
    ...
    native void iaEncode(ByteBuffer in, ByteBuffer out, int k, int m, int bufSize, ByteBuffer r);

    IAMigrationEncoder(int i){
      ...
      idx = i;
      seqIdx = stripeSize*bufSize;
    }
    //private?
    public void take(){
      //LOG.info("IAMigrationEncoder take called");
      inBuf.clear();
      ...
      LOG.info("anchor Encode_stripe "+inBuf.getInt(seqIdx)+" Start_encoding "+System.nanoTime());
      //LOG.info(inBuf.getInt(0)+" take check point: "+inBuf.get(5)+" "+inBuf.get(5+1024*1024)+" "+inBuf.get(5+1024*1024*2));
    }

    public void push(){
      //LOG.info("IAMigrationEncoder push called strange 3");
      ...
      for (int i = 0; i < paritySize; i++) {
        ...
        //LOG.info("port data index: "+i+" seq: "+seq+" bufsize: "+bufSize+" len: "+bufs[i].length);
    ...
    public void run(){
      LOG.info("run iamigrationencoder");
      while(true){
        //File[] fs = null;
        try{
          //take signal
        ...
        //}
        iaEncode(inBuf, outBuf, stripeSize, paritySize, bufSize, reserve);
